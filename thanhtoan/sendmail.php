...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $raw_price     = $_POST['product_price'] ?? '0';

    $clean_price     = preg_replace('/[^0-9]/', '', $raw_price);
    $formatted_price = number_format((float)$clean_price, 0, ',', '.') . ' VND';
...
<body style="font-family: Arial, sans-serif; color: #333; line-height: 1.5; background: #f9f9f9; padding: 20px;">
    <div style="max-width: 600px; margin: auto; background: #fff; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); padding: 20px;">
        ...
        <p><strong>Giá:</strong> ' . $formatted_price . '</p>';
        ...
        $mail->send();
        echo "<script>alert('Thông tin đã được gửi thành công!'); window.location.href='/Yadea';</script>";
        exit;
